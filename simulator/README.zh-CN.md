...
工具系统自带的 `samples/` 环境内留设了对主流话务体系涵盖范围面下的 PCAP 文件对照包参考：

| 文件名称 | 对应的具体运用业务行为体系 |
|------|----------|
| `basic_call.pcap` | 最纯粹的标准呼入响应循环过程。 (体现包含 `INVITE` → `200 OK` 流程节点)。 |
| `cancel_call.pcap` | 操作人发起但在系统或者对端确认处理动作之前取消挂断行为状态捕获 `CANCEL`。 |
| `reject_call.pcap` | 接收机器针对性执行挂停阻隔处理方案的抓取档案。 |
| `astercc_inner_5001_5002.pcap` | 面向 AsterCC 工程分机的内部分发数据通讯抓包流程体现包。 |
| `in_astcc_180-138-5001.pcap` | 系统获取并进行分配铃音及呼入动作。 |
| `in_wcc_*.pcap` | WCC 核心技术基础外延呼入业务各分发流向参考对象模型。 |
| `out_astcc_5001_180.pcap` | 对于 AsterCC 出向的真实逻辑发包数据参考件提取物。 |
| `out_wcc_*.pcap` | 面向 WCC 各级呼出场景并兼容大陆地区专用交互逻辑的特征对照体组合库。 |
| `wcc_5001-q1005A.pcap` | 经由系统排队队列进行坐席业务请求分发交互抓包日志记录还原体系参考文件。 |
