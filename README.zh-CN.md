...
| **管理后台 (Admin UI)** | [`admin-ui/`](admin-ui/) | React | 实时通话看板、AI 质检与数据分析大盘 |
| **Copilot 扩展** | [`copilot/`](copilot/) | React | Chrome AI 坐席辅助插件，实时转录与话术建议 |
...
- **隐私优先** — 边缘计算 PCI-DSS DTMF 脱敏，配合业务层 (AS) PII 脱敏清洗
...
> **性能基线**: 在实际 PCAP 压测中，单台 4 核 8GB 的 IE 实例可同时高效处理 300+ 路并发 SIP/RTP 呼叫 (G.711 PCMA/PCMU)，维持 CPU 资源占用率低于 60%。
